...
def plot_step_size_distribution(lag):
    ...
    for i in range(len(stepstaken)):
        for j in range(len(stepsmultilag)):
...
def makeImage(position):
    ...
    px = int(posx)
    py = int(posy)
    for i in range(max(0,px-border),min(len(data)-1,px+border),1):
        for j in range(max(0,py-border),min(len(data[i]-1),py+border),1):
